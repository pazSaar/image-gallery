...
const SearchBar: FC<SearchBarProps> = ({onChange}) => {
    ...
    const handleSaveFilter = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (savedFilters) {
            localStorage.setItem(savedFiltersLSKey, savedFilters + ',' + searchBarValue);
        } else {
    ...
    return (
        <div className="flex items-center justify-center">
            <input className="pl-2 m-1 w-52 self-center bg-gray-100 border-2 border-blue-300 rounded" type="text"
                 id="searchBar" onChange={handleSearchOnChange} value={searchBarValue}
            ...
            {togglePopupView ? <Popup togglePopup={togglePopup} listItems={localStorage.getItem(savedFiltersLSKey)?.split(",")}/> : null}
        </div>
    );
};
